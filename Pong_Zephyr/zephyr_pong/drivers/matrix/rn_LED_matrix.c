...
LOG_MODULE_REGISTER(rn_led_matrix, LOG_LEVEL_INF);
...
#define REFRESH_FREQUENCY K_MSEC(100)
...
static void led_matrix_refreshMatrix(struct k_work *work){
    int row, col;
    int pin_state = PIN_LOW;
    for(row = 0; row < 8; row++){
        for(col = 0; col < 8; col++){
            if(led_matrix[row][col] == 0){
                pin_state = PIN_LOW;
            }
            else {
                pin_state = PIN_HIGH;
            ...
            gpio_pin_set_dt(&(instance_config.rows[row]), pin_state);
            gpio_pin_set_dt(&(instance_config.cols[col]), pin_state);
        }
    }

    k_work_reschedule(&instance_config.refresh, REFRESH_FREQUENCY);
}
...
static int instance_init(const struct device *dev)
{
    ...
    k_work_init_delayable(&instance_config.refresh, led_matrix_refreshMatrix);
    k_work_schedule(&instance_config.refresh, REFRESH_FREQUENCY);
